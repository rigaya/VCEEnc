...
Run an ST-DeInt ONNX model for deinterlacing with ONNX Runtime DirectML. Input must be 8-bit YUV420 and its height must be an even value of at least 4. Available only in Windows builds with DirectML support.

The model must use the half-height, 6-channel restoration output generated by the current `export_stdeint.py`. Legacy models with ONNX weave output must be re-exported.

- **Parameters**
  - model=&lt;string&gt;  
    Registered ST-DeInt model name or path to an ONNX model (required). With `--vpp-onnx-model-dir`, the `stdeint` and `stdeint_fast` entries in `stdeint_ov_models.json` can be used.
  - mode=&lt;string&gt; (default: bob)  
    `bob` emits two frames in temporal order for each input and doubles the frame rate. `normal` emits only the frame corresponding to the first field.
  - device=&lt;string&gt; (default: GPU.0)  
    Compatibility parameter. DirectML binds inference to the same GPU as the encoder.
  - provider=&lt;string&gt; (default: auto)  
    VCEEnc supports `auto` (DirectML) only.
  - precision=&lt;string&gt; (default: fp32)  
    `fp32` and `auto` are accepted. Both run the model at its authored fp32 precision on DirectML.
  - colormatrix=&lt;string&gt; (default: auto)  
...
Progressive input passes through without pixel processing. With `mode=bob`, it is duplicated with the duration split in half to preserve the doubled-rate timeline. DirectML inference uses the host-copy path, shown as `path host` in the log.

Recommended combinations:

- High quality: `model=stdeint,precision=fp32`.
- Lower compute cost: `model=stdeint_fast,precision=auto`.
