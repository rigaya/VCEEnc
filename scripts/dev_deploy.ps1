...
<#
.SYNOPSIS
  VCEEnc をビルドし、AviUtl2 Plugin へ配置して起動する。
...
$Sln = Join-Path $RepoRoot "VCEEnc.sln"
...
if ($Platform -eq "x64") {
    $srcAuo2 = Join-Path $RepoRoot "_build\x64\$Configuration\VCEEnc.auo2"
} else {
    $srcAuo2 = Join-Path $RepoRoot "_build\Win32\$Configuration\VCEEnc.auo"
}
...
$dstAuo2 = Join-Path $PluginDir "VCEEnc.auo2"
Write-Host "==> copy"
Write-Host "    $srcAuo2"
Write-Host " -> $dstAuo2"
...
foreach ($name in @("VCEEnc.ini", "VCEEnc.en.ini", "VCEEnc.zh.ini")) {
