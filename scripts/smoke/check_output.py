#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""VCEEnc 終了バッチ用: 出力ファイル / ログ / aup2 設定のスモークチェック。"""
...
def extract_log_encoder_options(log_text: str) -> str | None:
    lines = log_text.splitlines()
    for idx, line in enumerate(lines):
        if "VCEEnc options" in line:
            buf: list[str] = []
            for fol in lines[idx + 1 :]:
                ...
                if not s:
                    ...
                    continue
                # 次セクションっぽい行で打ち切り
                if re.match(r"^(AAC|raw |VCEEnc \[|L-SMASH|CPU|encoded |converting |Aviutl)", s):
                    break
                if s.startswith("---"):
                    break
                buf.append(s.strip())
...
def compare_cli(expected: str, actual: str) -> list[str]:
    ...
    if not expected:
        return problems
    if not actual:
        problems.append("ログから VCEEnc options を抽出できませんでした")
        return problems
...
def main() -> int:
    ap = argparse.ArgumentParser(description="VCEEnc smoke check (after-bat)")
    ...
    name_keys = cfg.get("plugin_name_contains") or ["VCEEnc"]
    ...
    if not logpath.is_file():
        ...
        log_cli = extract_log_encoder_options(log_text)
        if log_cli:
            ok(f"ログ CLI 抽出: {log_cli[:120]}{'...' if len(log_cli) > 120 else ''}")
        else:
            ng("ログから VCEEnc options を抽出できませんでした")
        ...
        if aup2 and aup2.is_file():
            try:
                ...
                if conf is None:
                    ng(f"aup2 から VCEEnc 設定を見つけられません: {aup2}")
                else:
