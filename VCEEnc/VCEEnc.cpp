...
BOOL func_output( OUTPUT_INFO *oip ) 
{
    ...
    if (check_output(oip, &pe) && setup_afsvideo(oip, &g_sys_dat, &g_conf, &pe)) { //※3 start

        ret |= run_bat_file(&g_conf, oip, &pe, &g_sys_dat, RUN_BAT_BEFORE_PROCESS);

        for (int i = 0; !ret && i < 2; i++)
            ret |= task[g_conf.aud.audio_encode_timing][i](&g_conf, oip, &pe, &g_sys_dat);

        if (!ret)
            ret |= mux(&g_conf, oip, &pe, &g_sys_dat);

        ret |= move_temporary_files(&g_conf, &pe, &g_sys_dat, oip, ret);
    ...
    if (!(ret & (AUO_RESULT_ERROR | AUO_RESULT_ABORT)))
        ret |= run_bat_file(&g_conf, oip, &pe, &g_sys_dat, RUN_BAT_AFTER_PROCESS);
...
static BOOL check_output(const OUTPUT_INFO *oip, const PRM_ENC *pe) {
    ...
    if (g_conf.oth.out_audio_only)
        write_log_auo_line(LOG_INFO, "音声のみ出力を行います。");

    //音声エンコーダ
    if (oip->flag & OUTPUT_INFO_FLAG_AUDIO) {
        AUDIO_SETTINGS *aud_stg = &g_sys_dat.exstg->s_aud[g_conf.aud.encoder];
        if (str_has_char(aud_stg->filename) && !PathFileExists(aud_stg->fullpath)) {
            //fawの場合はfaw2aacがあればOKだが、それもなければエラー
            if (!(g_conf.aud.encoder == g_sys_dat.exstg->s_aud_faw_index && check_if_faw2aac_exists())) {
    ...
    switch (pe->muxer_to_be_used) {
        case MUXER_TC2MP4:
            check &= check_muxer_exist(&g_sys_dat.exstg->s_mux[MUXER_MP4]); //tc2mp4使用時は追加でmp4boxも必要
            //下へフォールスルー
        case MUXER_MP4:
        case MUXER_MKV:
            check &= check_muxer_exist(&g_sys_dat.exstg->s_mux[pe->muxer_to_be_used]);
            break;
        default:
            break;
    }
...
void open_log_window(const char *savefile, int current_pass, int total_pass) {
    ...
    show_log_window(g_sys_dat.aviutl_dir, g_sys_dat.exstg->s_local.disable_visual_styles);
...
static void set_tmpdir(PRM_ENC *pe, int tmp_dir_index, const char *savefile) {
    ...
    if (tmp_dir_index == TMP_DIR_CUSTOM) {
        //指定されたフォルダ
        if (DirectoryExistsOrCreate(g_sys_dat.exstg->s_local.custom_tmp_dir)) {
            strcpy_s(pe->temp_filename, _countof(pe->temp_filename), g_sys_dat.exstg->s_local.custom_tmp_dir);
            PathRemoveBackslash(pe->temp_filename);
            write_log_auo_line_fmt(LOG_INFO, "一時フォルダ : %s", pe->temp_filename);
        } else {
            warning_no_temp_root(g_sys_dat.exstg->s_local.custom_tmp_dir);
            tmp_dir_index = TMP_DIR_OUTPUT;
        }
    }
...
static void set_enc_prm(PRM_ENC *pe, const OUTPUT_INFO *oip) {
    ...
    //設定更新
    g_sys_dat.exstg->load_encode_stg();
    g_sys_dat.exstg->load_append();
    g_sys_dat.exstg->load_fn_replace();

    pe->video_out_type = check_video_ouput(&g_conf, oip);
    pe->muxer_to_be_used = check_muxer_to_be_used(&g_conf, &g_sys_dat, pe->temp_filename, pe->video_out_type, (oip->flag & OUTPUT_INFO_FLAG_AUDIO) != 0);
    memcpy(&pe->append, &g_sys_dat.exstg->s_append, sizeof(FILE_APPENDIX));

    char filename_replace[MAX_PATH_LEN];

    //一時フォルダの決定
    set_tmpdir(pe, g_conf.oth.temp_dir, oip->savefile);

    //音声一時フォルダの決定
    char *cus_aud_tdir = pe->temp_filename;
    if (g_conf.aud.aud_temp_dir)
        if (DirectoryExistsOrCreate(g_sys_dat.exstg->s_local.custom_audio_tmp_dir)) {
            cus_aud_tdir = g_sys_dat.exstg->s_local.custom_audio_tmp_dir;
            write_log_auo_line_fmt(LOG_INFO, "音声一時フォルダ : %s", cus_aud_tdir);
        } else
            warning_no_aud_temp_root(g_sys_dat.exstg->s_local.custom_audio_tmp_dir);
    ...
    g_sys_dat.exstg->apply_fn_replace(filename_replace, _countof(filename_replace));
...
static void auto_save_log(const OUTPUT_INFO *oip, const PRM_ENC *pe) {
    ...
    if (AUO_RESULT_SUCCESS != getLogFilePath(log_file_path, _countof(log_file_path), pe, &g_sys_dat, &g_conf, oip))
