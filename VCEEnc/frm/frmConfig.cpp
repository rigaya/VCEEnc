...
System::Void frmConfig::InitComboBox() {
	...
	setComboBox(fcgCXLSMVert,       list_vce_lsm_vert);
	setComboBox(fcgCXSubMode,       list_vce_rc_submode);
...
System::Void frmConfig::ConfToFrm(CONF_GUIEX *cnf) {
	...
	SetCXIndex(fcgCXEncMode,           get_cx_index(list_vce_rc_method, cnf->vce.rateControl.encRateControlMethod));
	SetNUValue(fcgNUBitrate,           cnf->vce.rateControl.encRateControlTargetBitRate / 1000);
	SetNUValue(fcgNUMaxkbps,           cnf->vce.rateControl.encRateControlPeakBitRate / 1000);
	SetNUValue(fcgNUQPI,               cnf->vce.rateControl.encQP_I);
	SetNUValue(fcgNUQPP,               cnf->vce.rateControl.encQP_P);
	SetNUValue(fcgNUQPB,               cnf->vce.rateControl.encQP_B);
	SetNUValue(fcgNUGopLength,    (int)cnf->vce.rateControl.encGOPSize);
	SetCXIndex(fcgCXCodecLevel,        get_cx_index(list_avc_level, cnf->vce.profileLevel.level));
	SetCXIndex(fcgCXCodecProfile,      get_cx_index(list_avc_profile, cnf->vce.profileLevel.profile));
	...
	SetNUValue(fcgNUSlices,             cnf->vce.pictControl.encNumSlicesPerFrame);

	fcgCBCABAC->Checked               = cnf->vce.pictControl.cabacEnable != 0;

	fcgCBDeblock->Checked             = cnf->vce.pictControl.loopFilterDisable == 0;
	SetNUValue(fcgNUDeblockStrength,    cnf->vce.pictControl.encLFAlphaC0Offset);
	SetNUValue(fcgNUDeblockThreshold,   cnf->vce.pictControl.encLFBetaOffset);

	SetCXIndex(fcgCXLSMVert,            get_cx_index(list_vce_lsm_vert, cnf->vce.meControl.lsmVert));
	SetNUValue(fcgNUMVSearchRangeX,     cnf->vce.meControl.encSearchRangeX);
	SetNUValue(fcgNUMVSearchRangeY,     cnf->vce.meControl.encSearchRangeY);
	fcgCBFMEAdvModeDecision->Checked  = cnf->vce.meControl.enableAMD != 0;
	SetCXIndex(fcgCXSubMode,            get_cx_index(list_vce_rc_submode, cnf->vce.meControl.encDisableSubMode));
	fcgCBForceZeroCenter->Checked     = cnf->vce.meControl.forceZeroPointCenter != 0;
...
System::Void frmConfig::FrmToConf(CONF_GUIEX *cnf) {
	//これもひたすら書くだけ。めんどい
	cnf->vce.rateControl.encRateControlMethod        = list_vce_rc_method[fcgCXEncMode->SelectedIndex].value;
	cnf->vce.profileLevel.profile                    = list_avc_profile[fcgCXCodecProfile->SelectedIndex].value;
	cnf->vce.profileLevel.level                      = list_avc_level[fcgCXCodecLevel->SelectedIndex].value;
	cnf->vce.rateControl.encRateControlTargetBitRate = (int)fcgNUBitrate->Value * 1000;
	cnf->vce.rateControl.encRateControlPeakBitRate   = (int)fcgNUMaxkbps->Value * 1000;
	cnf->vce.rateControl.encGOPSize                  = (int)fcgNUGopLength->Value;
	cnf->vce.rateControl.encQP_I                     = (int)fcgNUQPI->Value;
	cnf->vce.rateControl.encQP_P                     = (int)fcgNUQPP->Value;
	cnf->vce.rateControl.encQP_B                     = (int)fcgNUQPB->Value;
	cnf->vid.vce_ext_prm.pic_struct                  = (OVE_PICTURE_STRUCTURE_H264)list_interlaced[fcgCXInterlaced->SelectedIndex].value;
	cnf->vce.pictControl.encNumSlicesPerFrame        = (int)fcgNUSlices->Value;

	cnf->vce.pictControl.cabacEnable                 = fcgCBCABAC->Checked;

	cnf->vce.pictControl.loopFilterDisable           = !fcgCBDeblock->Checked;
	cnf->vce.pictControl.encLFAlphaC0Offset          = (int)fcgNUDeblockStrength->Value;
	cnf->vce.pictControl.encLFBetaOffset             = (int)fcgNUDeblockThreshold->Value;

	cnf->vce.meControl.encDisableSubMode             = list_vce_rc_submode[fcgCXSubMode->SelectedIndex].value;
	cnf->vce.meControl.enableAMD                     = fcgCBFMEAdvModeDecision->Checked;
	cnf->vce.meControl.encSearchRangeX               = (int)fcgNUMVSearchRangeX->Value;
	cnf->vce.meControl.encSearchRangeY               = (int)fcgNUMVSearchRangeY->Value;
	cnf->vce.meControl.lsmVert                       = list_vce_lsm_vert[fcgCXLSMVert->SelectedIndex].value;
	cnf->vce.meControl.forceZeroPointCenter          = fcgCBForceZeroCenter->Checked;
